...
"""
class ExperienceRepresentation(Representation):
    """
    ...
    def __init__(self):
        ...
        self._y = 0
        self._iteration = 0
        self._action_size = 100 # this is based on the probability of each tile
    ...
    def get_action_space(self, width, height, num_tiles):
        action_space = []
        for i in range(392):
            action_space.append(self._action_size)
    ...
    def get_observation_space(self, width, height, num_tiles):
        return spaces.Dict({
            ...
            "map": spaces.Box(low=0, high=self._action_size-1, dtype=np.uint8, shape=(height, width))
    ...
    def update(self, action, cur_block=0):
        change = 0
        self._x = cur_block * self._win_width
        self._y = 0
        ...
        for i in range(len(action)):
            # ----- With Probabilities -----
            tile = self.action_to_tile(action[i])
            if tile != -1:
                change += [0,1][self._map[now_y][now_x] != tile]
                self._map[now_y][now_x] = tile
            # ----- With Probabilities -----

            # ---- Equal Probabilities -----
            # action == 0 -> no change
            # if action[i] > 0:
            #     change += [0,1][self._map[now_y][now_x] != (action[i] - 1)]
            #     self._map[now_y][now_x] = action[i] - 1
            # ---- Equal Probabilities -----

            now_x += 1
            if now_x % self._win_width == 0:
                now_x = self._x
                now_y += 1
        
        # self._x = self.x_prev + 28
        # self._y = 0

        # # if reached the last block, go to the first block
        # if self._x >= self._width:
        #     self._x = 28

        return change, self._x, self._y

    def action_to_tile(self, action):
        #empty
        if 0 <= action <= 74:
            return 0
        #solid
    ...
    def render(self, lvl_image, tile_size, border_size):
        ...
        lvl_image.save("exp_rep_images/lvl_img_{}.png".format(self._iteration))
